r"""dna_ness.py -- character-fit's first netted substrate, and the worked exemplar.

A real, measured driven network: the Nicholas et al. fuel-driven DNA-NESS
(Angew. Chem. Int. Ed. 2025, 10.1002/anie.202512967; open PMC12535392), reduced to
a linear N=3 Markov generator (OQ, FQ, Q) and read for emergent identity -- a
protected NESS circulation that is *minted* by the drive, not stored.
  ...
  Coupling: Module B adds an irreversible FQ->Q pathway PARALLEL to A's reversible FQ<->Q, driving
    ...
    by the enzyme drive -- the structure pre-exists; the *protected current* is what coupling creates.

The rate constants are measured -- Nicholas SI Tables S4/S5 (Exp. Fig. 2b) + Sec. 3 --
and hardcoded below with their source (the SI is paywalled / returned 403, so the
numbers live in this file, per THE TRAPS). The independent confirmation that the N=3
reduction is faithful to the full nonlinear network is `dna_ness_crosscheck.py`.

Run:  python -m dna_ness.dna_ness   (or: python dna_ness/dna_ness.py)
"""
...
# --- THE SEAL (RECIPE step 2: written before the fit; do not edit to match output) ---
SEAL = """A detailed-balanced DNA-hybridization cycle (A=0 on its own) is driven by
RNase-H fuel hydrolysis into a protected, sustained NESS circulation:
  minted    = True   (Module A alone A=0, Module B alone A=0, coupled A!=0 with J!=0)
  protected = True   (sign of A is drive-locked; no reciprocal deformation flips it)
  sustained = True   (cut the drive -> detailed balance restored, J->0; nothing stored)
Expected verdict: MATCH -- using ONLY measured rates ([F],[O] cancel in the affinity)."""
...
def main():
    print("DNA-NESS -- character-fit's first netted substrate (Nicholas et al., Angew. 2025)\n")
    print("[SEAL] (written before the fit)\n" + SEAL + "\n")
